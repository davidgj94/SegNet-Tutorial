...
def get_iter(state):
    format_string = 'iter_{:0>9}.caffemodel'
    parsed = parse.parse(format_string, state.parts[-1])
    return int(parsed[0])

results_path = '../results/{}/'.format(sys.argv[1])
results_path_train = results_path + 'train/'
results_path_val = results_path + 'val/'
snapshot_dir = "../Models/Training/{}/".format(sys.argv[1])
...
if states:
    net = caffe.Net('../voc-fcn8s/deploy.prototxt', '../voc-fcn8s/fcn8s-heavy-pascal.caffemodel', caffe.TRAIN)
